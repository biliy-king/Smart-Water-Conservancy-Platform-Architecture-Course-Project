<template>
  <div class="chart-container">
    <div class="chart-title">上游水位监测</div>
  ...
  try {
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        // 筛选上游水位相关的实时数据（可能有多个测点）
        const upstreamDataList = realtimeData.filter(item => item.device_type === 'water_level_upstream')
        
        if (upstreamDataList.length > 0) {
          // 如果有多个测点，取平均值；如果只有一个，直接使用
          const avgValue = upstreamDataList.reduce((sum, item) => sum + item.value, 0) / upstreamDataList.length
...
async function loadHistoryData() {
  try {
    ...
    const waterLevelData = data.filter(item => 
      item.water_level_upstream !== null && item.water_level_upstream !== undefined
    )
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: function(params) {
        ...
        return `时间: ${time}<br/>上游水位: ${value !== null ? value.toFixed(3) : '--'} m`
      }
    },
    grid: {
      left: '12%',
